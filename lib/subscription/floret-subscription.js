...

{
    ...
    FloretSubscription = class FloretSubscription {
        constructor(name, service, router, gateway) {
            ...
            console.log('constructed subscription object')
        }

        async init() {
            console.log('looking to init subscription')
            await this.createSubscriptionAPI();
            await this.createSubscriptionEndpoint();
        }

        async createSubscriptionAPI(){
            console.log('url in createSubscriptionAPI ' + `${this[_service].baseURL}/subscription/${this[_name]}`)
            let gatewayAPIName = `${this[_service].name}_subscription_${this[_name]}`;
            let url =  `${this[_service].baseURL}/subscription/${this[_name]}`;


            // this[_endpoint] = url;
            this[_endpoint] = `${this[_gateway].proxyURL}/subscription/${this[_name]}`;
        ...
        async createSubscriptionEndpoint() {
            console.log('creating subscription endpoint')
            console.log(this[_uri]);
            this[_router].post( this[_uri], (msg) => {

...
let Floret, gatewayLib, subscriberLib, publisherLib;
{
    ...
    Floret = class Floret extends Koa {
        ...
        async createChannels(){
            await this[channels].map( this.createChannel );
        }

        async createChannel(channel){
            console.log('creating channel: ' + channel.name);
            // create endpoint at gateway
            await this[gateway].createChannelAPI(this[name] + '_channels_' + channel.name, `/${this[name]}/channels/${channel.name}/`, `${this.url}/${this[name]}/channels/${channel.name}` , 'GET').then((res) =>{

...

{
    ...
    Floret = class Floret extends Koa {
        ...
        // create a channel to broadcast new api specs to
        // we then subscribe the api-doc service to it
        // msg: new spec at
        // when floret stands up, need to attempt to send api spec info to
        // api document service

        async publishAPIDocumentation(newPaths=[]) {

            let apiSpec = this.buildAPISpec(newPaths);
            console.log('publishing docuemenation')
            this.registerAPI(`${this[_name]}-api-spec`, '/api-spec.json', 'GET,POST,DELETE').then( (res) => {
            ...
            // await channel.addSubscriber('api-docs');
            //await attachDocService(channel);
        ...
        buildAPISpec(newPaths=[]) {
            ...
            var swaggerDefinition = {
                info: { // API informations (required)
                    title: `${this.name}-api-spec`, // Title (required)

...

{
    ...
    Floret = class Floret extends Koa {
        ...
        createEnvConfig(config, envVars) {
            let envStr='local', host, port, name, uri, gatewayHost, gatewayAdminPort, gatewayProxyPort, disconnected;
            ...
            config.environments[envStr].host = (envStr === 'local') ? `${ip.address()}` : config.environments[envStr].host;

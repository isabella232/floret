...

{
    ...
    Floret = class Floret extends Koa {
        ...
        configure(config, ...[ConfigOverride, GatewayOverride, ChannelOverride, SubscriberOverride, SubscriptionOverride, ServiceOverride, PackageOverride]) {
            let {serviceName, serviceHost, servicePort, serviceURI, gatewayHost, gatewayAdminPort, gatewayProxyPort, documentationPaths} = config;
            this[_config] = config;
            this[_host] = serviceHost;
            this[_name] = serviceName;
            this[_port] = servicePort;
            this[_uri] = serviceURI || '/' + this[_name];
            this[_docPaths] = documentationPaths || [];
            ...
            this[_gateway] = new this[_gatewayClass]('gw', gatewayHost, gatewayAdminPort, gatewayProxyPort);
        ...
        async listen(portOverride) {
            ...
            if (!this.isInit) {
                ...
                return await this[_listen](portOverride || this[_port]);
            }
        }
        ...
        async init() {
            ...
            try {
                console.log(`Initializing floret [${this[_uri]}]...`);
                ...
                this.use(async (ctx, next) => {
                    console.log('body parser in vfloret')
        ...
        async registerAPI(name, uri, methods) {
            await this[_gateway].addAPI(name, [this[_uri] + uri], this[_baseURL] + uri, methods).then(() => {
        ...
        async publishAPIDocumentation(newPaths = []) {
            ...
            console.log('sending api request to refresh docs ')
        ...
        async initChannels() {
            ...
            console.log('channels collected: ' + this[_channels].length);
        ...
        async initChannel(channel) {
            ...
            console.log('step 1 compelte')
            await this.discoverSubscribers(channel);
            console.log('finish init of channel ' + channel.name)
        }
        ...
        async addChannel(channel) {
            ...
            this[_channels][channel.name] = {};
            this[_channels]['testest'] = {};
            this[_channels][channel.name] = channel;

            //this.channels = this[_channels];

            console.log('so now I set this[_channels] ' + JSON.stringify(this[_channels]['api-spec'].name) )
            //return channel;
        }
...
*/
        ...
        async discoverChannels() {
            ...
            console.log('resgisteresd hcae ' + registeredChannels)
        ...
        async createChannelAPI(channel) {
            ...
            await this[_gateway].createChannelAPI(channel.apiName, `/${this[_name]}/channels/${channel.name}/`, `${this[_service].baseURL}${channel.uri}`, 'GET, POST')
                .then((res) => {
                    console.log('channelAPI created');
                }).catch((e) => {
                    if (e.statusCode !== 409) {
        ...
        async createSubscriberAPI() {
            console.log('creating subscriber api')

...
let Floret;
{
    ...
    Floret = class Floret extends Koa {
        ...
        async createSubscriberAPI() {
            await this[gateway].addAPI(this[name] + '_subscribers', [this[uri] + '/subscribers'], this[host] + ':' + this[port] + '/' + this[name] + '/subscribers', 'GET,POST,OPTIONS').then(() => {
            ...
            this[appRouter].post('/subscribers/', async(ctx, next) => {
                ...
                ctx.body = await this[gateway].addAPI(subscriberName, [subURI], subRequest.url, 'GET,POST,OPTIONS')
                    .then((res) => {
                        return res;
                    }).catch((e) => {
                        if (e.statusCode !== 409) {
                ...
                .delete('/subscribers/:channelName/:subscriberName', (ctx) =>{
                    this.unsubscribe(ctx.params.channelName, ctx.params.subscriberName);
                    // remove from api gateway
                    this.gateway.deleteAPI(ctx.params.subscriberName);
                })
        ...
        unsubscribe(channelName, subscriberName) {
            this[channels][channelName].unsubscribe(subscriberName);
        }

        subscribe(name, channel, fn){

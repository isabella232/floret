...

{
    ...
    Floret = class Floret extends Koa {
        ...
        configure(floretConfig, ...[ConfigOverride, GatewayOverride, ChannelOverride, SubscriberOverride, SubscriptionOverride, ServiceOverride, PackageOverride]) {
            ...
            this[_host] = this[_config].serviceHost;
            this[_name] = this[_config].serviceName;
            this[_port] = this[_config].servicePort;
            this[_uri] = this[_config].serviceURI || '/' + this[_name];
        ...
        async init() {
            ...
            try {
                ...
                console.log('...healthcheck created ');
                ...
                if (!this[_disconnected]) {
                    ...
                    console.log('...gateway registration complete');
        ...
        async registerAPI(name, uri, methods) {
            return await this[_gateway].addAPI(name, [this[_uri] + uri], this[_baseURL] + uri, methods).then(() => {
        ...
        configureAPIs(apis) {
            apis.map( (api) => {
                if (api.path) {
                    require(`../../../${api.path}`)(this);
                }
            });

            this[_registeredAPIs] = this[_registeredAPIs].concat(apis);
        }
        /**
         *
         * @param name
...
/*
         ...
         * @param channelsMap
         *
        set channels(channelsObj) {
            this[_channels] = channelsObj;
        }
*/
        /**
         *
        ...
        async subscribe(serviceName, channelName, subscription) {
            ...
            console.log(serviceName, channelName, subscription.name, subscription.endpoint);
        ...
        createConfig (cfg, host, port, service, uri, gatewayHost, gatewayAdminPort, gatewayProxyPort, disconnected) {
            return new this[_configClass](cfg, host, port, service, uri, gatewayHost, gatewayAdminPort, gatewayProxyPort, disconnected);
        }

        createEnvConfig(config, envVars) {
            let envStr = envVars.FLORET_ENV_KEY || 'local';
            let host = envVars.FLORET_HOST;
            let port = envVars.FLORET_PORT;
            let service = envVars.FLORET_NAME;
            let uri = envVars.FLORET_URI;
            let gatewayHost = envVars.FLORET_GATEWAY_HOST;
            let gatewayAdminPort = envVars.FLORET_GATEWAY_ADMIN_PORT;
            let gatewayProxyPort = envVars.FLORET_GATEWAY_PROXY_PORT;
            let disconnected = envVars.FLORET_IS_DISCONNECTED;

            config.environments[envStr].gatewayHost = (envStr === 'local') ? `${ip.address()}` : config.environments[envStr].gatewayHost;
            let envObj = Object.assign({}, config.environments[envStr]);
            delete config.environments;
            return this.createConfig (Object.assign({}, config, envObj), host, port, service, uri, gatewayHost, gatewayAdminPort, gatewayProxyPort, disconnected);
        }

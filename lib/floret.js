    ...
    Floret = class Floret extends Koa {
        ...
        async init() {
            ...
            try {
                ...
                console.log('Endpoint uris prefixed with ' + this[uri]);

                await this[gateway].gatewayHealthCheck();
                console.log('gateway is alive.');

                // register service with api gateway
                console.log('registering heatbeat api with gateway');
                let api = await this[gateway].register(this[name], this[uri], this[host] + ':' + this[port]+ '/' + this[name] + '/healthcheck', 'GET');
                ...
                console.log(JSON.stringify(this[channelPublishers]));
                ...
                this.healthCheck();
                console.log('healthCheck api created.')
        ...
        async createChannels(){
            for (let i=0; i< this[channels].length; i++){
                await this.createChannel(this[channels][i])
            }
        }

        async pauseChannel(name){

            this[appRouter].get('/channels/' + name + '/pause', (ctx) => {
                console.log('channel request ');

                ctx.body = {
                    name: this[channelPublishers][name].name,
                    paused: this[channelPublishers][name].pause ? true : false
                };
            });

            this[appRouter].post('/channels/' + name + '/pause', (ctx) => {
                console.log('channel request ');
                this[channelPublishers][name].pause = Object.create(this[channelPublishers][name].handler.prototype);
                this[channelPublishers][name].handler = () => {};
            });
        }

        async resumeChannel(name){
            this[channelPublishers][name].handler = Object.create(this[channelPublishers][name].pause.prototype);
            delete this[channelPublishers][name].pause;
        }
        ...
        static localParams() {
            let GATEWAY_URL = "http://127.0.0.1",
                GATEWAY_PROXY_PORT = 8000,
                GATEWAY_ADMIN_PORT = 8001,
                CONTAINER_PORT = 8091,
                CONTAINER_HOST = "http://127.0.0.1",
                SERVICE_NAME = "pulse",
                SERVICE_BASE_URI = "/pulse";

            return [GATEWAY_URL, GATEWAY_PROXY_PORT, GATEWAY_ADMIN_PORT,CONTAINER_PORT, CONTAINER_HOST, SERVICE_NAME, SERVICE_BASE_URI];
        }
    }

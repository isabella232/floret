...
const Subscription = require('floret-subscription');
...
let Floret;
{
    ...
    Floret = class Floret extends Koa {
        ...
        constructor(serviceName, serviceHost, servicePort, serviceURI, gatewayURI, gatewayAdminPort, gatewayProxyPort, routePrefix, ...options) {
            ...
            this[_subscriptionClass] = options[3] || Subscription;
            this[_serviceClass] = options[4] || Service;
            this[_handlerClass] = options[5] || Handler;
         }
        ...
        async createSubscriptionAPI(name, handler){
            let gatewayAPIName = `${this[_name]}_subscription_${handler.name}`;
            let url =  `${this[_service].baseURL}/subscription/${name}`;
            this[_gateway].addAPI(gatewayAPIName, [`/${this[_name]}/subscription/${name}`], url, 'POST');
        }

        async createSubscriptionEndpoint(subscriptionName) {
            this[_appRouter].post( `/subscription/${subscriptionName}`, async(ctx, next) => {
        ...
        async discoverSubscriptions() {
            let subscriptions = await this[_gateway].discover(`${this.name}_subscription_`);

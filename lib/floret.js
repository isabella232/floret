    ...
    Floret = class Floret extends Koa {
        ...
        async init() {
            ...
            try {
                ...
                let api = await this[gateway].register(this[name], this[uri], this.url + '/' + this[name] + '/heartbeat', methods);
                ...
                console.log('retrieving existing subscriber list');
                
                this[subscribers] = await this[gateway].loadSubscribers(this[name]);
                console.log('subscribers received: ' + this[subscribers].length);

                // this[defaultChannel] = await this[gateway].createChannel('default', '/channel/default');
                //await this[gateway].createChannelAPI('default', '/channel/default');
                ...
                console.log('Error occurred during floret init routine.' + e.message);
                console.log(e.stack);
                throw e;
            }
        }
        ...
        async listen(portOverride) {
            ...
            await this.init().then(() => {
                super.use(this[appRouter].routes());
        ...
        async subscriberEventsIn() {
            ...
            this[appRouter].post('/subscribers/', async (ctx, next) => {
                console.log('incoming subscriber event');
                ...
                console.log('interpolation: ' + `\/${this[name]}\/subscribers\/${event.name}${event.uri}`);
                ctx.body = await this[gateway].addAPI(event.name + '_subscribers', `\/${this[name]}\/subscribers\/${event.name}${event.uri}`, this.url + event.uri, 'GET,POST,OPTIONS')
                    .then((res) => {
            ...
            try {
                subscriberApi = await this[gateway].addAPI(this[name] + '_subscribers', this[uri] + '/subscribers', this.url + '/' + this[name] + '/subscribers', 'GET,POST,OPTIONS');

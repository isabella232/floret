...

{
    ...
    FloretChannel = class FloretChannel {
        constructor({ name, endpoint, uri, serviceName, description }, handler, broadcaster) {

            let config = { name, endpoint, uri, serviceName, description };
            this[_name] = name;
        ...
        subscribe(subscriber){
            console.log('creating a subscriber with key : ' + subscriber.name)
        ...
        broadcast(payload, sender = this.name , trackingId) {
            ...
            if (this.subject.observers.length > 0) {
                console.log('observer length is ' + this.subject.observers.length)
                console.log('channel is ' + this[_name]);
        ...
        defaultEventHandler(name, endpoint, broadcaster, params) {
            ...
            if (endpoint) {
                ...
                console.log('emitting to endpoint ' + endpoint);

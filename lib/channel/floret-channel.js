    ...
    FloretChannel = class FloretChannel {
        ...
        subscribe(subscriber){
            ...
            if (!this[_subscribers][subscriber.name] && subscriber.endpoint) {
                let observer = this[subject].subscribe( this[_handler].bind(null, that));
        ...
        unsubscribe(subscriberName){
            this[_subscribers][subscriberName].unsubscribe();
        ...
        get subscribers() {
            console.log('returning subscribers for me: ' + this.name)
        ...
        incomingMessages(ctx, msg) {
            console.log('Incoming message to channel: ' + ctx.name);
            ...
            for(let key in ctx[_subscribers]){
                ...
                if (endpoint) {
                    ...
                    ctx.send(options).then(() => {
                        console.log('sent from subscriber ' + name)
                    }).catch((e) => {
                        if (e.statusCode !== 409) {
                            console.log('status was actuall ' + e.statusCode)
                            throw e;
                        }
                    });
                }

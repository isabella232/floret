    ...
    FloretGateway = class FloretGateway {
        ...
        async register(service) {
            ...
            if (res && res.message === "Not found") {
                api = await this.addAPI(service.name, service.proxyURI + '/healthcheck', this.ensureProtocol(service.baseURL) + '/healthcheck', 'GET,POST,PUT,PATCH,UPDATE,DELETE,OPTIONS');
                status = 'new'
            } else {
                status = 'existing';
